...
\begin{itemize}
  \item Problemi \textbf{decisionali}: output Yes/No a seconda che l'input soddisfi o meno una determinata proprietà.
  \item Problemi di \textbf{ricerca}: si cerca una \textit{soluzione ammissibile} (ammissibile = che soddisfa una certa condizione)
...
sequenza finita di passi (altrimenti il codice sorgente non sarebbe finito), ordinata, non ambigua,
...
Un algoritmo corretto ritorna sempre la soluzione corrispondente per ogni istanza di input.
...
Il costo computazionale \textbf{worst case} di un \textit{algoritmo} è il numero di operazioni elementari necessarie per risolvere
il problema su qualunque istanza, compresa la peggiore.\\ 
...
La difficoltà nel trattare un problema dipende dalla sua complessità computazionale:
\begin{itemize}
 ...
 \item Problemi \textbf{irrisolvibili}: si può dimostrare che non esiste un algo per il problema.
\end{itemize}
...
I problemi chiaramente intrattabili sono, ad esempio, quelli che richiedono un output di dimensione più che polinomiale rispetto all'input
...
(esponenziale) nella dimensione dell'input (ovvero il numero di dischi presenti).
...
La teoria della complessità categorizza i problemi nelle diverse classi. Ma\dots le classi di questi problemi, dipendono dal modello di calcolo?
\theobox{Tesi di Curch-Turing (1936): modelli di calcolo diversi sono simulabili a vicenda con slowdown polinomiale.
Ovvero, tutti i modelli di calcolo sono polinomialmente equivalenti alla Macchina di Turing.}
Quindi\dots l'appartenenza alle classi non viene stravolta dal \textbf{modello di calcolo}.
