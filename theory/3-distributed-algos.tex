...
Questo campo dell'informatica nasce 'per caso' insieme all'esigenza di costituire cluster di computazione con le risorse disponibili.
Ad esempio, i computer dei docenti in un dipartimento universitario.
Le topologie di queste reti sono inizialmente le più svariate, spesso i collegamenti sono disordinati e non pensati.

Def. Ambiente distribuito. Collezione finita di entità che scambiano messaggi per un obiettivo comune.

Gli algoritmi distribuiti sono chiamati anche protocolli... infatti un protocollo è un algoritmo distribuito sul almeno due nodi.

Vogliamo che il protocollo sia corretto, ovvero che risolva il problema richiesto, e che sia efficiente, ovvero che lo faccia con un basso costo.
...
aumentando i nodi aumentare la potenza).

I diversi ambienti su cui un algo può girare sono:
 - sequenziale: single core, single memory
 - parallelo: multicore, shared memory, small distances
 - distribuito: many processors, many memories, parallel operations, wide distances among entities.
...
costantemetne messaggi e sono collaborativi.

Le entità dell'ambiente distribuito sono gli elementi che eseguono la computazione. Queste entità hanno una certa autonomia, a livello di
clock e memoria, per esempio, ed interagiscono tra loro scambiandosi messaggi.

Lo stato di una entità x, indicato con status(x), registra lo stato di x in ogni momento. Lo stato può appartenere ad un insieme
finito di stati. Lo stato deve essere sempre definito.

Le entità sono reattive: reagiscono agli eventi con un comportamento. Se non accade nulla, non viene fatto niente.
Esempi di eventi sono messaggi e tick del clock.

Il comportamento (behaviour) B(x) di una entità x è l'insieme di regole di una entità x che associano ad ogni stato e ogni evento un'azione.
B deve essere deterministico e completo.

Il comportamento del sistema è B(E) = {B(x) | x \in E}, con E set di entità.
Il comportamento deve essere simmetrico, ovvero ogni enità deve rispettare lo stesso set di regole.

Ogni entità riceve messagi solamente dai suoi in-neighbors e manda messaggi solamente ai suoi out-neighbors.

La comunicazione tra le entità può essere unidirezionale o bidirezionale.
La topologia della rete può essere modellata con un grafo.
Un grafo diretto modella una comunicazione unidirezionale, mentre un grafo adiretto modella la bidirezionalità.

Assiomi.

Def. Orientamento locale. Ogni nodo è in grado di disambiguare tra i messaggi che riceve da i suoi vicini.
Distingue chi gli ha mandato un messaggio e può scegliere a chi mandarlo.

Le restrizioni sono proprietà aggiuntive che limitano l'applicazione di un protocollo.
E.g., direzionalità dei link, comunicazione FIFO o unordered, edge and entity failures
(failure detection, limiti di tolleranza, come continuare dopo un fault, ecc.).

Reliability restrictions:
- total reliability: tutto funziona sempre e per sempre
- partial reliability: possono esserci stati fallimenti in passato, ma non ce ne saranno in futuro
- guaranteed delivery: ogni messaggio viene sicuramente consegnato, ma non ho garanzie sulle entità

Altre restrizioni tipiche sono:
 - connettività: eg., grafo strongly connected
 - grado di conoscenza della rete: conoscenza sul numero di link, nodi e diametro della rete
 - delay di comunicazione: bounded se ci sono garanzie sul tempo di ricezione, unbounded altrimenti
 - delay unitario: delay costante anche in caso di failure
 - sincronicità dei clock: tutti i clock sono sincronizzati

Complessità per gli algo distribuiti.s
In questi algoritmi il tempo è molto importante, mentre lo spazio su ogni nodo è poco interessante.
Molto più interessante considerare la quantità di messaggi scambiati.

A volte conteremo il numero di messaggi scambiati, a volte il numero di bit.

Tempo.
Come contare la complessità temporale di questi algoritimi?
Idealmente: le unità di tempo che servono per ricevere il risultato, se il sincronismo tra le entità fosse totale.
In questo caso si parla di ideal time complexity.
Problema: il delay di comunicazione non è predicibile.
Soluzione: usiamo il numero di messaggi scambiati come misura di costo temporale. Consideriamo la catena di messaggi più lunga.
Questa funzione di costo tiene in considerazione l'asincronismo tra i clock delle entità. Viene chiamata casual time complexity.

Stiamo completamente ignorando il tempo necessario ai nodi per l'elaborazione. Tempo trascurabile rispetto al tempo di comunicazione.

Il tempo è misurato dall'esterno come periodo tra l'inizio della computazione alla fine della stessa.
